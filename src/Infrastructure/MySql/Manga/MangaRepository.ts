...
export class MangaRepository implements IMangaRepository {
	...
	async getAll(): Promise<Manga[]> {
		try{
			...
			return rows.map((row) => {
				const mangaRow = row as MySqlMangaWithAuthor; // casteo implicito de mysqlrow a mi firma. Si hay que hacer casteo explicito, aca
				return this.mangaMapper.toEntityFromMangaWithAuthor(mangaRow);
			});
		} catch (error: any) { throw new RepositoryException("Error en la base de datos MySql: " + error.message); }
	}

	async getByTitle(title: string): Promise<Manga | null> {	
		try{
			const sql = `
			SELECT 
				m.id, 
			...
			if (rows.length === 0) {
				...
				const mangaRow = rows[0] as MySqlMangaWithAuthor;
				return this.mangaMapper.toEntityFromMangaWithAuthor(mangaRow);
			}
		} catch (error: any) { throw new RepositoryException("Error en la base de datos MySql: " + error.message); }
	}
	...
	async insertOne(manga: Manga): Promise<Manga> {
		try {
			const mangaModel: MySqlManga = this.mangaMapper.toMangaFromEntity(manga);
